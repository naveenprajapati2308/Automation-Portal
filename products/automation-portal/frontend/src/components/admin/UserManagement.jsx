...
export function UserManagement({ setNotice }) {
  ...
  return (
    <section className="page-grid" style={{ gridTemplateColumns: '1fr' }}>
      ...
      {/* ── Modals ── */}
      {showCreate && (
        <AdminModal title="Create New User" onClose={() => setShowCreate(false)}>
          <CreateUserForm
            setNotice={setNotice}
      ...
      {editUser && (
        <AdminModal title={`Edit User — ${editUser.username}`} onClose={() => setEditUser(null)}>
          <EditUserForm
            user={editUser}
            setNotice={setNotice}
      ...
      {resetTarget && (
        <AdminModal title={`Reset Password — ${resetTarget.username}`} onClose={() => setResetTarget(null)}>
          <ResetPasswordForm
...
// ── Admin Modal — does NOT close on backdrop click ─────────────────────────────
function AdminModal({ title, children, onClose }) {
  return (
    <div className="modal-overlay" /* no onClick here — only X closes it */>
