...
export function DataTable({
  ...
  // Windowed page-number buttons: all pages if few, else first/last + a
  // sliding window around the current page with '...' gaps.
  const pageNumbers = useMemo(() => {
    if (totalPages <= 7) return Array.from({ length: totalPages }, (_, i) => i + 1);
    const pages = [1];
    if (safeCurrentPage > 3) pages.push('…');
    for (let p = Math.max(2, safeCurrentPage - 1); p <= Math.min(totalPages - 1, safeCurrentPage + 1); p++) pages.push(p);
              ...
              {columns.map(col => {
                ...
                return (
                  <th
                    ...
                    className={col.sticky === 'right' ? 'datatable-col-sticky' : undefined}
              ...
              Array.from({ length: pageSize }).map((_, rIdx) => (
                <tr key={rIdx}>
                  {columns.map(col => {
                    if (!visibleColumns[col.key]) return null;
                    return (
                      <td key={col.key} className={col.sticky === 'right' ? 'datatable-col-sticky' : undefined}>
          ...
          <div className="datatable-pagination-controls">
            ...
            <div className="datatable-page-buttons">
              ...
              {pageNumbers.map((p, i) => p === '…' ? (
                <span key={`gap-${i}`} className="datatable-page-gap">…</span>
              ) : (
                <button
                  key={p}
                  onClick={() => setCurrentPage(p)}
                  className={`datatable-page-number${p === safeCurrentPage ? ' active' : ''}`}
